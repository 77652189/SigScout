...
SigScout 是一个面向分泌表达设计的信号肽工作台，用于发现、解释、筛选、聚类和导出候选信号肽。当前默认演示项目是人骨桥蛋白 OPN 在毕赤酵母/Komagataella 中的分泌构建设计。
...
- 目标蛋白成熟氨基酸序列，例如 OPN 去除天然信号肽后的序列。
...
src/sigscout/presets/opn.py OPN 默认预设
...
- UniProt 数据作为公开数据库来源使用，仍需在对外材料中保留 accession、来源数据库和检索条件。
