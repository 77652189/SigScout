...
from sigscout.adapters.uniprot import UniProtSignalPeptideSource
from sigscout.core.inputs import CandidateInputProvider, REQUIRED_CANDIDATE_INPUT_COLUMNS
...
REQUIRED_IMPORT_COLUMNS = REQUIRED_CANDIDATE_INPUT_COLUMNS
...
class SignalPeptideLibraryService:
    ...
    input_provider: CandidateInputProvider | None = None

    def __post_init__(self) -> None:
        candidates = list(self.candidates)
        self.input_errors: list[str] = []
        self.input_source_name = "static candidates"
        if self.input_provider is not None:
            batch = self.input_provider.load_candidates()
            candidates.extend(batch.candidates)
            self.input_errors = list(batch.errors)
            self.input_source_name = batch.source_name
        self._candidates = candidates

    @classmethod
    def from_input_provider(
        cls,
        input_provider: CandidateInputProvider,
        *,
        candidate_prefix: str = "UNIPROT",
    ) -> "SignalPeptideLibraryService":
        return cls(input_provider=input_provider, candidate_prefix=candidate_prefix)
        ...
        writer.writerow(
            {
                ...
                "source_note": "请填写 UniProt/文献/外部工具/内部实验来源",
    ...
    def discover_uniprot_candidate_library(
        ...
        *,
        ...
        return UniProtSignalPeptideSource(self._candidates, candidate_prefix=self.candidate_prefix).discover(
            taxon_id=taxon_id,
    ...
    def rows_from_uniprot_payload(self, payload: dict) -> tuple[list[dict[str, object]], list[str]]:
        return UniProtSignalPeptideSource(self._candidates, candidate_prefix=self.candidate_prefix).rows_from_payload(payload)
