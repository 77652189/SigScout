...
这些候选不是赢了引导分——**它们根本没参加那场评分**。对照通道是**独立的一次取用**，
用自己的排序键、拿自己的配额，所以引导分没有机会淘汰它们，前三个通道也挤不掉它。

**而且它不是象征性的负例。** 排序有两级：先按与低表现锚点的相似度从高到低，
同样相似时再取**不依赖反馈的证据最强**的那一条。也就是说，这一通道挑的是
**"看起来最有希望、却坐落在已知失败邻域里"**的候选——于是两种结果都有信息量：

| 结果 | 说明什么 |
| --- | --- |
| 它成了 | 通用证据是对的；那条锚点的失败另有原因，不是邻域的问题 |
| 它没成 | 邻域是对的；规则存在盲区，而现在你知道盲区在哪 |

如果第二级改取**证据最弱**的，那失败是必然的，**什么都学不到**。
这是被设计过的判别实验，不是凑一个负例。
