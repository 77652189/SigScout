...
没有排队中的 UX Phase 了（A/B/C 三步都做完，见上方记录）。剩下的都是可选、非阻塞项：

1. **顺带发现、还没处理的可选清理项**（都不在已完成 Phase 的范围内，只是顺带记录）：
...
2. **六个代码重构 Phase + 三个 UX Phase 一致的教训**：写执行计划时凭经验列出的清单，动手前必须用代码里的真实依赖关系（以及目标框架/库的实际运行行为——代码重构 Phase 1 的 `ui/pages/` 命名冲突、UX 计划里 `st.Page` 不能带参数都是这么发现的）去核实，不能直接当真——细节见 [EXECUTION_PLAN.md](EXECUTION_PLAN.md) 开头的"跨 Phase 通用教训"。
