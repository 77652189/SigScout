...
2. **UI 层零自动化测试**（见下方"已知的坑"第 3 条）——如果想补，需要先决定测试策略（比如用 Streamlit 自带的 `AppTest` 框架写页面级冒烟测试，还是别的方案），这是一个独立的小型项目，不是几行代码能解决的。
3. **六个代码重构 Phase + 三个 UX Phase 一致的教训**：写执行计划时凭经验列出的清单，动手前必须用代码里的真实依赖关系（以及目标框架/库的实际运行行为——代码重构 Phase 1 的 `ui/pages/` 命名冲突、UX 计划里 `st.Page` 不能带参数都是这么发现的）去核实，不能直接当真——细节见 [EXECUTION_PLAN.md](EXECUTION_PLAN.md) 开头的"跨 Phase 通用教训"。
...
3. **UI 层零自动化测试**：`ui/streamlit_app.py`、`ui/_shared.py`、`ui/views/*.py`、`ui/experimental_browser.py` 没有任何 pytest 覆盖，Phase 1 拆分后依然如此，UX 三个 Phase 也都是靠手动 Streamlit 走查验证的。改 UI 之后 `pytest -q` 全绿不代表没坏，必须手动跑一遍 Streamlit 页面。**2026-07-29 评估过**：要补的话需要先选测试策略（比如 Streamlit 自带的 `streamlit.testing.v1.AppTest` 可以不开浏览器跑页面级冒烟测试），这是一个独立的小项目，没有跟着这次可选清理一起做。
...
5. **`core/paths.py` 的姊妹属性 `opn_screening_output_dir`（`local_runs/opn_signal_peptides`）也带同样的历史命名残留**（2026-07-29 清理 `opn_saved_screening_dir` 时顺带发现）：跟已经改名的 `opn_saved_screening_dir` 是同一类问题（跟目标蛋白无关，纯粹是"毕赤酵母信号肽筛选"输出目录，历史遗留带了 opn 字样），但引用点有 5 处（`cli.py`/`_shared.py`/`fusion_localization.py`×2/定义处），比刚处理的那个多，这次没有一并动。低优先级，顺手改的时候改，注意改名不要碰实际磁盘路径字符串。
6. **新建 UI 子目录时不要叫 `pages`**：这个名字被 Streamlit 保留给它自己的多页面应用自动发现机制，撞了会在侧边栏冒出一份多余的导航列表。SigScout 目前用 `ui/views/` 存放页面渲染模块。
