...
| `fusion_constructs.py` | 635 | `build_fusion_constructs`（多目标预设 `FUSION_TARGET_PRESETS` 下生成 AC/ABC 构建）、`score_construct`（6 个子打分：signal_detail/source_context/processing/risk/localization_probability/fine_priority）、`summarize_localization`（DeepLoc 概率分桶）、`import_localization_results`（DeepLoc/BUSCA CSV/TSV 解析）、`fusion_constructs_to_csv`/`_to_fasta` | 构建/打分/定位导入/导出四类职责混在一个文件；`to_csv`/`to_fasta` 与 `exports.py` 的写法本质重复（[EXECUTION_PLAN.md](EXECUTION_PLAN.md) Phase 3，未开始） |
